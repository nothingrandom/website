function getAge(e){var t=new Date;aSecond=1e3,aMinute=60*aSecond,aHour=60*aMinute,aDay=24*aHour,aWeek=7*aDay,aMonth=30*aDay;var a=t.getTime()-e.getTime();return years=new Date(t.getTime()-aMonth*e.getMonth()).getYear()-new Date(e.getTime()-aMonth*e.getMonth()).getYear(),offsetNow=new Date(t.getTime()-aDay*(e.getDate()-1)),offsetBirth=new Date(e.getTime()-aDay*(e.getDate()-1)),months=years>1?12*years+(offsetNow.getMonth()-offsetBirth.getMonth()):12*(t.getYear()-e.getYear())+(offsetNow.getMonth()-offsetBirth.getMonth()),agestr="",months<24?(weeks=Math.floor(a/aWeek),a-=weeks*aWeek,days=Math.floor(a/aDay)):(agestr+=years,agestr=agestr),agestr}
...
window.onload=function(){document.getElementById("copyyear").innerHTML=copyright(2013),$("#age").length>0&&(document.getElementById("age").innerHTML=getAge(new Date("November 3, 1997")))},$(document).ready(function(){if($("#lastfm").length>0){$(".top-artists").css("display","block");var e="user.getTopArtists",t="nothingrandom",n="928f184254c119b939f618cdbf36e58d",a="1month",o="3";$.getJSON("http://ws.audioscrobbler.com/2.0/?method="+e+"&user="+t+"&api_key="+n+"&period="+a+"&limit="+o+"&format=json&callback=?",function(e){var t="";$.each(e.topartists.artist,function(e,n){t+="<li><a href="+n.url+" target='_blank'>"+n.name+"</a></li>"}),$("#lastfm").append(t)})}if($("#instafeed").length>0){var i=new Instafeed({get:"user",limit:12,links:!0,resolution:"low_resolution",sortBy:"most-recent",template:'<div class="box"><a href="{{link}}"><img src="{{image}}" /><div class="hover"><h4>{{caption}}</h4></div></a></div>',userId:896545907,accessToken:"896545907.1210342.dcf80e3b731848189d8ed763572cca5d"});i.run()}});
